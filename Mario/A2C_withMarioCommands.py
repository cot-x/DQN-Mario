...
class Mario:
    ...
    ACT_NONE = [0,0,0,0,0,0]
    ACT_LEFT = [0,1,0,0,0,0]
    ACT_RIGHT = [0,0,0,1,0,0]
    ACT_DASH_LEFT = [0,1,0,0,0,1]
...
class A2C(nn.Module):
    ...
    def loss(self, states, actions, rewards, action_evals, estimated_values):
        ...
        actions = torch.LongTensor(actions).to(action_evals.device)
...
class Environment:
    def __init__(self, args):
        ...
        self.agent = Agent(args.cpu, args.lr, args.mem_capacity, args.batch_size, Mario.ACT_COUNTS)
    ...
    def train(self):
        ...
        for i in tqdm(range(self.args.num_updates)):
            ...
            buttons = np.array(Mario.ACTIONS[action.item()])
            (state_next, _), reward, done, _ = self.env.step(buttons)
    ...
    def save_movie(self):
        env = self.make_env_play(self.args.env_name)
        ...
        while True:
            action = self.agent.get_action(state)
            buttons = np.array(Mario.ACTIONS[action.item()])
            (state, frame), _, done, _ = env.step(buttons)
            frames += [frame[0]]
            if done:
                break
